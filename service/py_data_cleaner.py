...
class DataCleanerPy:
    ...
    def run(self) -> dict[int, bool]:
        ...
        """
        ad_client_ids = []
        rows = DB_Interface.fetch_many(f'SELECT ad_client_id from {SCHEMA}.ad_client order by ad_client_id')
        for row in rows:
            ad_client_ids.append(row[0])
        # ad_client_ids = [1000005]
        ...
        if pipeline_state_list:
            PipelineState.persist_pipeline_state_list(self.db_api, pipeline_state_list)
        return state_map
    ...
    def run_for_ad_client(self, ad_client_id: int, queue: Queue) -> bool:
        with ProcessPoolExecutor(max_workers=self.workers) as executor:
            ...
            try:
                ...
                aggregated_df = AggregationPipeline(ad_client_id).run()
                # aggregated_df = pd.DataFrame()
                if aggregated_df is not None and not aggregated_df.empty:
                    self.clear_aggregated_data(ad_client_id)
                    with DB.get_isolated_connection() as conn:
                        with conn.cursor() as cursor:
                            if not DB_Interface.copy_from_df(aggregated_df,
                                                             AggregationPipeline.table_name,
                                                             cursor):
                                raise Exception(f'Could not persist data into {AggregationPipeline.table_name}')
                    conn.close()
    ...
    def process_chunks(self,
        ...
        conn = DB.get_isolated_connection()
        ...
        select = f'SELECT date_run FROM {SCHEMA}.data_cleaning_client_stats '\
        ...
        date_last_run = self.db_api.fetch_one(select, (ad_client_id,), conn=conn)

        if date_last_run is not None:
            if where_clause is not None:
                where_clause += " AND "
            where_clause += "created >= to_timestamp('" + str(date_last_run[0]) + "', 'YYYY-MM-DD HH24:MI:SS')::timestamp"

        df = self.db_api.get_data_for_pipeline(pipeline=pipeline_obj, where_clause=where_clause,
                                               columns=pipeline_obj.columns_to_use, conn=conn)
        total_chunks, total_rows, result_rows = 0, 0, 0
        table = pipeline.value.table_name
        time_run = None
        ...
        with conn:
            with conn.cursor() as cursor:
                for chunk in df:
                    ...
                    if not DB_Interface.copy_from_df(cleaned_chunk,
                                                     table,
                                                     cursor):
                        raise Exception(f'Could not persist data into {table}')
                time_run = datetime.timedelta(seconds=(time.time() - start_time))
    ...
    def clear_aggregated_data(self, ad_client_id: int):
        self.db_api.execute_statement(f'DELETE FROM {SCHEMA}.cleaned_aggregated_data WHERE AD_Client_ID = %s',
                                      (ad_client_id,))
        ...
        for k, v in state_dict.items():
            ...
            for pipeline_result in v:
                client_state['runtime_in_seconds'] = client_state[
                                                         'runtime_in_seconds'] + pipeline_result.time_run.total_seconds()
                if pipeline_result.pipeline == Pipeline.INVOICE:
                    ...
                    if pipeline_result.result_rows != 0:
                        client_state['invoices_remaining_percent'] = (pipeline_result.result_rows / pipeline_result.total_rows) * 100
                    ...
                    if pipeline_result.result_rows != 0:
                        client_state['alloc_remaining_percent'] = (pipeline_result.result_rows / pipeline_result.total_rows) * 100
                    ...
                    if pipeline_result.result_rows != 0:
                        client_state['terms_remaining_percent'] = (pipeline_result.result_rows / pipeline_result.total_rows) * 100
                    ...
                    if pipeline_result.result_rows != 0:
                        client_state['payments_remaining_percent'] = (pipeline_result.result_rows / pipeline_result.total_rows) * 100

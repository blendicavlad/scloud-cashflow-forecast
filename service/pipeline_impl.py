...
class InvoiceCleaningPipeline(AbstractPipeline):
    initial_table_name = 'c_invoice'
    table_name = 'c_invoice_cleaned'
...
class AllocationCleaningPipeline(AbstractPipeline):
    initial_table_name = 'c_allocationline'
    table_name = 'c_allocationline_cleaned'
...
class PaymentCleaningPipeline(AbstractPipeline):
    initial_table_name = 'c_payment'
    table_name = 'c_payment_cleaned'
...
class PaymentTermCleaningPipeline(AbstractPipeline):
    initial_table_name = 'c_paymentterm'
    table_name = 'c_paymentterm_cleaned'
...
class AggregationPipeline(AbstractPipeline):
    table_name = 'cleaned_aggregated_data'
    __FEATURES = ['closed_late_invoices_no',
                  'paid_late_percent',
                  'paid_late_total',
                  ...
                  'paymentrule',
                  'tendertype'] \
                 + [x + '_platit' for x in ['max', 'min', 'avg', 'std']] \
                 + [x + '_platit_late' for x in ['max', 'min', 'avg', 'std']] \
                 + [x + '_unpaid' for x in ['max', 'min', 'avg', 'std']] \
                 + [x + '_unpaid_late' for x in ['max', 'min', 'avg', 'std']]
    ...
    def run(self) -> DataFrame:
        invoice_df = DB_Interface.get_data_to_df(sql=f'SELECT * FROM {SCHEMA}.c_invoice_cleaned'
                                                     f' WHERE ad_client_id = ' + str(self.ad_client_id))
        allocations_df = DB_Interface.get_data_to_df(sql=f'SELECT * FROM {SCHEMA}.c_allocationline_cleaned'
                                                         f' WHERE ad_client_id = ' + str(self.ad_client_id))
        payments_df = DB_Interface.get_data_to_df(sql=f'SELECT * FROM {SCHEMA}.c_payment_cleaned'
                                                      f' WHERE ad_client_id = ' + str(self.ad_client_id))
        terms_df = DB_Interface.get_data_to_df(sql=f'SELECT * FROM {SCHEMA}.c_paymentterm_cleaned'
                                                   f' WHERE ad_client_id = ' + str(self.ad_client_id))
    ...
    def generate_aggregated_cleaned_df(self, allocations_df, invoice_df, payments_df, terms_df):
        ...
        invoice_and_terms_df = invoice_and_terms_df[invoice_and_terms_df.dateinvoiced <= invoice_and_terms_df.duedate]
        ...
        invoice_terms_and_payments_df.drop(columns=['ad_org_id_y', 'c_bpartner_id_y', 'c_currency_id_y', 'duedate_y'],
                                           inplace=True)
        ...
        cash_paid3 = invoice_terms_payments_allocations_df[invoice_terms_payments_allocations_df.c_cashline_id_y != 0]
    ...
    @staticmethod
    def __build_training_data(d_param: DataFrame) -> DataFrame:
        ...
        df = AggregationPipeline.__build_derived_features(closed_invoices, df)
    ...
    @staticmethod
    def __build_derived_features(closed_invoices, df):
        start_time = time.time()
        ...
        df['avg_days_paid_late'] = df.groupby(['c_bpartner_id'])['days_late_closed_invoices_late'].transform('sum') / \
...
# class EndPipeline() ???? Daca nu poate fi pus tot in AggregationPipeline
